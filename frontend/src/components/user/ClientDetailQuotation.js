...
const ClientDetailQuotations = () => {
    ...
    const clientOptions = orderData.map(order => ({ value: order.client, label: order.client }));
      ...
      return (
        <div>
            <div className="buttonaddquotation w-100 text-end">
                <Button color="primary" className="btn-sm" onClick={toggle}>Add Quotation</Button>
            </div>
        <table className="table table-orders">
          <thead className="tb-odr-head">
            <tr className="tb-odr-item">
              <th className="tb-odr-info">
                <span className="tb-odr-id">Quotation ID</span>
              </th>
          ...
          <tbody className="tb-odr-body">
            ...
            <DropdownMenu end>
              <ul className="link-list-plain">
                <li> <DropdownItem tag="a" href={`/quotation-details/${item.id}`} > View </DropdownItem> </li>
                <li> <DropdownItem tag="a" href={`/quotation-print/${item.id}`}  > Download </DropdownItem> </li>
...
export default ClientDetailQuotations;